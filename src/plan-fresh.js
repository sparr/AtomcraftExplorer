...
function assemble(graph, spec, procs, index, supply, x, fetchTotal, sub, notes) {
  ...
  const priming = [];
  {
    ...
    const worth = (got) => {
      ...
      return [got.charge.size, paid];
    };
    let laid = layIn(new Set());
    const refused = new Set();
    for (const name of [...laid.charge.keys()].sort()) {
      if (!laid.charge.has(name)) continue;         // an earlier refusal took it
      const without = layIn(new Set([...refused, name]));
      if (without.charge.has(name)) continue;       // it would not do without
      const [n0, p0] = worth(laid);
      const [n1, p1] = worth(without);
      if (n1 < n0 || (n1 === n0 && p1 < p0)) { laid = without; refused.add(name); }
    }
