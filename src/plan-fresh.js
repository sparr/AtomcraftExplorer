...
function assemble(graph, spec, procs, index, supply, x, fetchTotal, sub, notes) {
  ...
  const priming = [];
  {
    ...
    /**
     * And refusing one thing turns up others to refuse.
     *
     * Refusing the Carbon Monoxide puts Carbon on the list, and Carbon was not
     * on the list to begin with, so a single pass over the first answer never
     * reaches it -- it stops at nine Carbon where three will do. Each round
     * tries every charge now standing, and rounds run until one changes
     * nothing.
     */
    for (let round = 0; round < 8; round++) {
      let moved = false;
      for (const name of [...laid.charge.keys()].sort()) {
        ...
        const without = layIn(new Set([...refused, name]));
        if (without.charge.has(name)) continue;     // it would not do without
        const [p0, n0] = worth(laid);
        const [p1, n1] = worth(without);
        if (p1 < p0 || (p1 === p0 && n1 < n0)) { laid = without; refused.add(name); moved = true; }
      }
      if (!moved) break;
    }
