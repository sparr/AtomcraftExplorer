...
function assemble(graph, spec, procs, index, supply, x, fetchTotal, sub, notes) {
  ...
  const priming = [];
  {
    ...
    const madeIn = (got) => wants.reduce((a, n) => a + rnum(got?.stock?.get(n) || R0), 0);
    const dryWith = (hold, times) => layIn(refused, caps, { hold, times });
    const full = [1, 2].map((t) => madeIn(dryWith(laid.charge, t)));
    const buys = new Map();
    for (const name of laid.charge.keys()) {
      const without = new Map([...laid.charge].filter(([n]) => n !== name));
      const lost = [1, 2].map((t, i) => full[i] - madeIn(dryWith(without, t)));
      // Costing the same at both sizes means it is a first-cycle lag; growing
      // with the plant means the plant cannot do without it.
      buys.set(name, { lag: lost[0], holdsUp: lost[1] > lost[0] + 1e-9 });
    }
