...
/**
 * Paint a material's shading onto a node, stepping through its frames when the
 * delegate animates. Used for both the list icons and the detail tile, so they
 * share one strip in the cache and one speed.
 */
function paintPattern(node, m) {
  const strip = patternStrip(m, { size: ICON_TILE, frames: ANIM_FRAMES });
...
function patternPreview(m) {
  ...
  if (!paintPattern(box, m)) return null;
...
function renderDetail(m) {
  ...
  // Only when the material actually emits. Materials.cs defaults LightRange to
  // 0 where it is unset, and Gameplay reads that range to decide lighting, so a
  // colour without a range lights nothing -- 76 materials carry such a value,
  // and showing it claimed every powered logic gate glows red.
  if (r.LightRange) {
