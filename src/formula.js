...
/**
 * Element counts for arithmetic, where `counts` is for search.
 *
 * `tally` counts every branch of an alternate site at full weight, because a
 * search for niobium should find `Fe(Ta,Nb)2O6`. That is the wrong answer for
 * anything that has to add atoms up: read literally, columbite carries two
 * tantalum *and* two niobium, and a plan built on it mints metal.
 *
 * Here each branch instead takes an equal share of the site, which is the only
 * assumption available without knowing the deposit -- and for columbite it is
 * the one the chemistry agrees with, since dissolving it yields tantalic and
 * niobic acid one for one. Shares can be fractional: `(Cs,Na)AlSi2O6` holds
 * half a caesium.
 */
function share(items, mult, out) {
  let pending = mult;
  for (const node of items) {
    switch (node.k) {
      case 'coeff':
      ...
      case 'group':
        for (const branch of node.branches) {
          share(branch, (node.n * pending) / node.branches.length, out);
        }
        break;
      case 'sep':
        if (SEGMENT_BREAKS.has(node.text)) pending = mult;
...
/**
 * What one unit of a material is made of, as an element -> count map, with
 * alternate sites split evenly. Returns `counts` unchanged when there is no
 * such site, so the common case costs nothing.
 */
export function expectedCounts(parsed) {
  if (!parsed || !parsed.counts.size) return null;
  if (!parsed.alternates.size) return parsed.counts;
  const out = new Map();
  share(parsed.ast, 1, out);
  return out;
}
