 ...
 */
import { rnum, rat, radd, rsub, rmul, rdiv, rcmp, rmin, R0 } from './rational.js';
...
export function planGraph(plan, { materials = false, foldPhases = true } = {}) {
  ...
  for (const step of plan.steps) {
    ...
    const node = { id, kind: STEP, name: step.process.label,
                   ...
                   kindOf: step.process.kind, packs };
    seen.set(id, node);
    nodes.push(node);
  }
  ...
  if (materials) {
    ...
    for (const b of plan.byproducts) {
      ...
      put(wants, b.name,
          { id: pseudo(`spare:${b.name}`, said(b.name), 'spare').id, amount: b.amount });
    }

    /**
     * A charge goes straight to the reaction it was laid in to start, and is
     * taken out of that reaction's demand before anything is shared out. It is
     * not part of the pool: it is the one push that gets the wheel turning,
     * and it belongs to one reaction by construction.
     */
    const owedTo = new Map();                 // "name\u0000stepId" -> amount charged
    for (const c of plan.priming) {
      for (const one of c.forSteps || []) {
        ...
        const key = `${c.name}\u0000${at}`;
        owedTo.set(key, radd(owedTo.get(key) || R0, one.amount));
      }
    }
    for (const [key, amount] of owedTo) {
      const at = key.indexOf('\u0000');
      const list = wants.get(key.slice(0, at)) || [];
      for (const w of list) {
        if (w.id !== key.slice(at + 1)) continue;
        w.amount = rsub(w.amount, rmin(w.amount, amount));
      }
    }
  ...
  if (!materials) {
    ...
    for (const e of edges) {
      ...
      const end = [e.from, e.to].find((x) => seen.get(x)?.pseudo);
      e.label = end
        ? (arrowsAt.get(end) > 1 ? `${rnum(e.amount)}` : '')
        : `${rnum(e.amount)} ${e.label}`;
    }
    join();
  }
