...
export function planGraph(plan, { materials = false, foldPhases = true } = {}) {
  ...
  if (materials) {
    ...
    for (const name of new Set([...makers.keys(), ...users.keys()])) {
      ...
      /**
       * Sparr: rx is much messier, owing to combining all the inputs, outputs
       * and primers.
       *
       * It is, and that is what a hub does. Every material the reader brings
       * arriving from one box makes that box a star of a dozen arrows, and a
       * star has to cross everything to reach reactions spread across the
       * picture. Four of them, and the crossings were mostly theirs.
       *
       * So each end-material gets a little box of its own -- which is what
       * made the materials drawing readable, and costs nothing here because
       * these are the ones that already had a single end. The materials in
       * the middle stay on the arrows, having a reaction at either end.
       */
      const amount = amountOf(name);
      const said = amount === null ? name : `${amount} ${name}`;
      ...
      if (wanted.has(name) && from.length) {
        ...
        for (const [a] of from) edges.push({ from: a, to: sink.id, role: 'want' });
      } else if (spare.has(name) && from.length) {
        const sink = pseudo(`spare:${name}`, said, 'spare');
        for (const [a] of from) edges.push({ from: a, to: sink.id, role: 'spare' });
      }
    }
  }
